...
$(function () {
    ...
    */
    ...
    //变化函数
    function change(num){//change($obj1,$obj2,num)
...
//top-list start
$(function () {
    var $title=$(".top-list .title li");
    var $content=$(".top-list .menu-content li");
    var now=0;
    //变化函数
    function change(num){
        $content.eq(now).fadeOut(2000);
        $title.eq(now).removeClass("active");
        now=num;
        $content.eq(now).fadeIn(2000);
        $title.eq(now).addClass("active");
    }
    $title.click(function () {
        var num = $(this).index();
        change(num);
    //    change($content,$title,num)
    });
});
//top-list end
